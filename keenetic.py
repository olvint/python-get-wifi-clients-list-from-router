...
tn.read_until(b"(show)> ")                            # Awaiting for router to ask for login (Note capital "L")
tn.write("ip arp".encode('ascii') + b"\n")            # Sending command "ip arp" to be able to input command
output= tn.read_until(b"(show)> ").decode('ascii')    # Awaiting for router to write wifi clients
